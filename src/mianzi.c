...
Mianzi **huleMianziQiduizi(Shoupai *shoupai){
    ...
    for(int i = 0; i < 4; i++){
        for(int j = 1; j <= 9; j++){
            ...
            // 対子の場合
                ...
                // 和了牌と一致するかどうかを確認
                if(i == shoupai->hulepai->paizhong && j == shoupai->hulepai->value) mianzi[count] = initializeMianzi(Duizi, i, j, false, true, shoupai->hulepai->value - j, shoupai->hulepai->source);
    ...
    // 和了牌にマークをつける
        ...
        for(int i = 0; i < 7; i++){
            if(mianzi[i]->isHulu) continue;
            if(mianzi[i]->paizhong == shoupai->hulepai->paizhong && mianzi[i]->value == shoupai->hulepai->value){
                mianzi[i]->isHulu = true;
                break;
            }
            ...
            for(int k = 0; k < combinationCount; k++){
                ...
                for(int l = 0; l < 5; l++){
                    if(compMianzi[k][l]->paizhong == shoupai->hulepai->paizhong){
                        ...
                        huleMianzi[count + offset][l]->source = shoupai->hulepai->source;
                        offset++;
                    }
                }
            }
...
Mianzi ***huleMianzi(TeshuDamanguan *flag, Shoupai *shoupai){
    // 国士無双, 九蓮宝燈の確認
    *flag = huleMianziGuoshi(shoupai);
    if(*flag != None) return NULL;

    *flag = huleMianziJiulian(shoupai);
    if(*flag != None) return NULL;

    Mianzi ***mianzi = huleMianziYiban(shoupai);
    Mianzi **mianziQiduizi = huleMianziQiduizi(shoupai);
