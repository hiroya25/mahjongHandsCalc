...
Yi *createYi(char *name, uint8_t fanshu, Sijia baojia){
    ...
    yi->name = strdup(name);
    if(yi->name == NULL){
        fprintf(stderr, "メモリ確保に失敗しました。サイズ: %zu, 型のサイズ: 1\n", sizeof(name));
        return NULL;
    }
    yi->fanshu = fanshu;
    yi->baojia = baojia;
...
void concatYiArray(Yi **data1, Yi **data2){
    ...
    for(int i = 0; i < data2Count; i++){
        data1[data1Count + i] = createYi(data2[i]->name, data2[i]->fanshu, data2[i]->baojia);
    }
...
Yi **getPostHupai(Shoupai *shoupai, uint8_t *baopai, uint8_t *fubaopai){
    ...
    for(int i = 0; fubaopai[i] % 10 != 0; i++){
        ...
        for(int j = 0; newShoupai->fulou[j] != NULL; j++){
            ...
            if(newShoupai->fulou[j]->paixing == Shunzi){
                // 順子のとき、value, value + 1, value + 2のいずれかで、ドラ+1
                if(baoVal == newShoupai->fulou[j]->value || baoVal == newShoupai->fulou[j]->value + 1 || baoVal == newShoupai->fulou[j]->value + 2) nFubaopai++;
            }else{
                // 順子以外の時、valueで、ドラ+3 or +4
                if(baoVal == newShoupai->fulou[j]->value){
                    switch(newShoupai->fulou[j]->paixing){
 ...
 * @param huleHand (bool *) 和了可能な手牌であるかどうか
...
Hule *hule(bool *huleHand, Shoupai *shoupai, JuInfo *juInfo){
    ...
    *huleHand = false;
    ...
    while(mianzi[count] != NULL){
        ...
        if(hupai == NULL) { count++; continue; } // hupaiがないとき、スキップ
        *huleHand = true;    // 和了可能な手牌のとき、マーク
        if(hupai[0] == NULL) { count++; free(hupai); continue; } // 役がないとき、スキップ
        ...
        uint8_t fanshu = 0, damanguan = 0; uint32_t defen = 0;  // 点数に関する変数
        Fengpai baojia2 = 0; uint32_t defen2 = 0;               // パオに関する変数
        ...
        // パオがあった場合
        if(defen2 > 0){
            if(shoupai->hulepai->source != Zimo) defen2 /= 2;   // ロン和了の場合は放銃者と折半
        ...
        if(shoupai->hulepai->source != Zimo || defen == 0){
            // ロン和了もしくはパオ1人払いの場合
            Fengpai baojia = defen == 0 ? baojia2 : (juInfo->menfeng + shoupai->hulepai->source) % 4; // パオ1人払いは放銃者扱い
            ...
            // ツモ和了の場合  
            uint32_t zhuangjia = ceil(defen * 2 / 100.0) * 100;  // 親の負担額
            uint32_t sanjia    = ceil(defen     / 100.0) * 100;  // 子の負担額
        ...
        // 得られた和了点の最大値を解とする
        if(damanguan > max->damanguan ? true : defen + defen2 > max->defen ? true : fanshu > max->fanshu){
            // 元々のhupaiを解放
        ...
        //--- 解放 ---//
        free(hudi); // 順子の面子構成はシャローコピーなので変数のみ解放
        if(hupai != NULL) freeYiArray(hupai); // hupaiの解放
        count++;
    }

    //--- 解放 ---//
    freeYiArray(preHupai); // すべてディープコピーしたのですべて解放

    // 役満のときは、すべてfree, 通常の時は、シャローコピーなので変数のみ解放
    if(max->damanguan > 0) freeYiArray(postHupai);
    else free(postHupai);

    freeAllMianziArray(mianzi); // 副露面子はディープコピーしたので全て解放
